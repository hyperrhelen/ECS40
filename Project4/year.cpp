...
Year& operator+= (Year &year, Day &days )
{
  short month = days.getmonth();
  short day = days.getday();
  int i;
  int flag = 0;
  for (i = 0 ; i < year.count; i++)
    if(year.days[i].compareDate(month,day) == 0)
    {
      flag = 1;
      break;
    }
